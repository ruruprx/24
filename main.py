...
from discord import app_commands, utils # ★ app_commandsを追加
...
# コマンドグループ /joinlog を定義
log_group = bot.tree.group(name="joinlog", description="入室/退室ログチャンネルを設定します。")

@log_group.command(name="set", description="入室・退室ログを送信するチャンネルを設定します。")
@app_commands.describe(channel="ログを送信するチャンネル")
@app_commands.default_permissions(administrator=True) # 管理者権限をデフォルトで要求
...
@log_group.command(name="disable", description="入室・退室ログの送信を無効にします。")
@app_commands.default_permissions(administrator=True) # 管理者権限をデフォルトで要求
...
# プレフィックスコマンド: !ban (省略しますが、機能は前回のコードと同じです)
